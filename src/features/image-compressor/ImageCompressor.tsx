...
interface ImageBatchItemResult {
  id: string;
  originalFile: File;
...
export const ImageCompressor: React.FC = () => {
  const [queue, setQueue] = useState<ImageBatchItem[]>([]);
  const [results, setResults] = useState<Record<string, ImageBatchItemResult>>({});
  ...
  const updateItemSettings = (id: string, updates: Partial<ImageBatchItem>) => {
    setQueue((prev) =>
      prev.map((item) => {
        if (item.id === id) {
          return { ...item, ...updates };
  ...
  const executeBatch = async () => {
    ...
    setZipBlob(null);
    setZipProgress('Starting image batch compilation...');
    ...
    const newResults: Record<string, ImageBatchItemResult> = {};
    ...
    for (let i = 0; i < validatedQueue.length; i++) {
      ...
      setZipProgress(`Compressing image ${i + 1} of ${queue.length}: ${item.customName}`);
      ...
      // drasitc warning check
      let isDrastic = false;
      if (targetKB < originalKB * 0.15) {
        isDrastic = true;
      }
  ...
  const triggerZipDownload = () => {
    ...
    link.href = objectUrl;
    link.download = `freeSeva_images_package_${Date.now().toString().slice(-6)}.zip`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(objectUrl);
  ...
  return (
    <PageLayout
      title="Image Compressor Workspace"
      description="Compress and package multiple image files concurrently. Select formats, rename files, and download everything in one compiled ZIP."
    >
        ...
        {queue.length === 0 && (
          <div className="flex flex-col gap-6 max-w-4xl mx-auto w-full">
            <div className="text-center max-w-xl mx-auto mb-2 select-none">
              <h2 className="text-lg font-black text-navy-950 font-display">Upload images to compress:</h2>
              <p className="text-xs text-navy-500 font-semibold mt-1">
                Upload one or multiple images (JPG, PNG, WEBP). Customize target limits and filenames in the workspace.
              </p>
            </div>
            ...
            <DragDropUpload
              ...
              helperText="Supports JPG, JPEG, PNG, and WEBP formats up to 15MB each"
            />
          </div>
        )}
              ...
              <div className="w-full overflow-x-auto pb-2 scrollbar-thin">
                      ...
                      >
                        ...
                        <div className="flex items-center gap-4 w-full lg:w-auto">
                          ...
                          <div className="w-12 h-12 rounded-lg bg-navy-200 flex-shrink-0 overflow-hidden border border-navy-300 flex items-center justify-center shadow-sm relative select-none">
                            ...
                            <span className="absolute bottom-0 inset-x-0 bg-black/60 text-[8px] text-white text-center py-0.2 select-none" title={`Original footprint size: ${formatBytes(item.file.size)}`}>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full lg:flex lg:flex-row lg:items-center lg:justify-between lg:w-auto lg:gap-6 flex-1 border-t lg:border-t-0 border-navy-100/70 pt-3 lg:pt-0">
                          ...
                          <div className="min-w-[110px] select-none">
                            ...
                            <span className="inline-block px-2.5 py-1 text-[10px] font-extrabold uppercase rounded-lg border border-brand-200 bg-brand-50 text-brand-750 tracking-wide">
                              {item.file.type.includes('png') ? 'PNG Image' : (item.file.type.includes('webp') ? 'WEBP Image' : 'JPG Image')}
                            </span>
                          </div>
                          ...
                          <div className="min-w-[195px]">
                            <div className="flex flex-col gap-1">
                              ...
                              {item.sizeMode === 'range' ? (
                                <div className="flex items-center gap-1 bg-white border border-navy-200 rounded-lg px-2 py-0.5 max-w-[185px]">
                                  <input
                                    type="number"
                        ...
                        {/* Right Group: Row Actions (Enclosed perfectly inside card border) */}
                ...
                <div className="mt-4 border-t border-navy-150 pt-4 flex flex-col sm:flex-row gap-4 items-center justify-between">
                  <div className="flex gap-2 items-center text-xs font-semibold text-navy-500 select-none">
                    ...
                    <span>Click 'Compress Image Documents' to execute image size compression constraints.</span>
                  </div>
            ...
            <div className="flex flex-col gap-2 mt-2">
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <button
                  ...
                  className="flex-grow px-4 py-2.5 bg-navy-50 hover:bg-navy-100 border border-navy-250 text-navy-700 font-bold text-xs rounded-xl transition-all cursor-pointer text-center select-none font-semibold animate-fadeIn"
                >
                  Increase Limit
                </button>
              </div>
